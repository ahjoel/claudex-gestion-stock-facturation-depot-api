...
class ProduitRepository {
  async save(produit) {
    return await db.claudexBarsDB.query(
      "INSERT INTO produits (code, name, description, model_id, fournisseur_id, pv, seuil, created_by, created_at) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, now());",
      [
        produit.code,
        produit.name,
        produit.description,
    ...
    return await db.claudexBarsDB.query(
      `UPDATE produits
             SET code             = CASE WHEN ? IS NOT NULL THEN ? ELSE code END,
                 ...
                 seuil          = CASE WHEN ? IS NOT NULL THEN ? ELSE seuil END,
                 updated_by        = ?,
                 updated_at        = now()
             WHERE id = ?`,
      [
